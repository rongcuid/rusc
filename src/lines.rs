...
impl TryFrom<&Path> for FileFormat {
    ...
    fn try_from(path: &Path) -> Result<Self> {
        ...
        match ext {
            ...
            "" => Ok(FileFormat::Text),
            ext => Err(miette!("unsupported extension `.{ext}`")),
        }
    }
}
...
pub fn open_lines_input(path: Option<impl AsRef<Path>>) -> Result<LineReader> {
    match path {
        ...
        Some(input) => match input.as_ref().try_into().unwrap() {
            ...
            FileFormat::Gzip => {
                let reader = MultiGzDecoder::new(File::open(input).into_diagnostic()?);
                let bufr = BufReader::new(reader);
                Ok(Box::new(bufr.lines()))
            }
            FileFormat::Lz4 => {
                let reader =
                    lz4_flex::frame::FrameDecoder::new(File::open(input).into_diagnostic()?);
                Ok(Box::new(BufReader::new(reader).lines()))
            }
            FileFormat::Zstd => {
                let reader = zstd::stream::read::Decoder::new(File::open(input).into_diagnostic()?)
                    .into_diagnostic()?;
                Ok(Box::new(BufReader::new(reader).lines()))
            }
        },
    ...
    let writer: Box<dyn Write> = match path {
        ...
        Some(output) => match output.as_ref().try_into().unwrap() {
            ...
            FileFormat::Lz4 => Box::new(BufWriter::new(
                lz4_flex::frame::FrameEncoder::new(File::create(output).into_diagnostic()?)
                    .auto_finish(),
            )),
            FileFormat::Zstd => Box::new(BufWriter::new(
                zstd::stream::write::Encoder::new(File::create(output).into_diagnostic()?, 0)
                    .into_diagnostic()?
                    .auto_finish(),
            )),
        },
